...
//or copy the codes
//© 2022 Xeon Bot Inc. Asia/Colombo
const {
    WAConnection,
    MessageType,
    Presence,
...
const starts = async(DogeXeonOP = new WAConnection()) => {
    ...
    console.log(color(figlet.textSync('Asia/Colombo', {
        font: 'Standard',
    ...
    console.log(color('> WA NUMBER: +94767184785 ', 'silver'))
    ...
    console.log(color('<>', 'red'), color('But? Error? Suggestion? Visit Here:', 'aqua'), color('https://wa.me/94767184785'))
    console.log(color('[Asia/Colombo]'), color('Asia/Colombo Is Online', 'aqua'))
    console.log(color('[DEV]', 'cyan'), color('Welcome Back Owner! Hope You Doing Well~', 'magenta'))
    console.log(color('<>', 'red'), color('Thanks For Using Asia/Colombo', 'white'))
    ...
    DogeXeonOP.on("group-participants-update", async(anu) => {
        ...
        try {
            ...
            if (anu.action == "add" && mem.includes(DogeXeonOP.user.jid)) {
                DogeXeonOP.sendMessage(anu.jid, "Hello everyone, am TQ Bot, ready to help you here. \n Type| *.menu* |list all commands", "conversation")
            }
            ...
            if (anu.action == 'add' && !mem.includes(DogeXeonOP.user.jid)) {
                ...
                welcomeButt = { contentText: `${teks}`, footerText: `𝑻𝒆𝒄𝒉𝒏𝒚 𝑸𝒖𝒆𝒆𝒏`, buttons: welcomeBut, headerType: 6, locationMessage: bosco2.message.locationMessage }
            ...
            if (anu.action == 'remove' && !mem.includes(DogeXeonOP.user.jid)) {
                ...
                goodbyeButt = { contentText: `${out}`, footerText: `𝑻𝒆𝒄𝒉𝒏𝒚 𝑸𝒖𝒆𝒆𝒏`, buttons: goodbyeBut, headerType: 6, locationMessage: bosco3.message.locationMessage }
